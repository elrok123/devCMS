<?php include('../../../dbaccess/index.php');  include('../../../dbaccess/modules.php');
	...
	if(isset($_POST['hiddenDelete']))
	{
		...
		$clientToDelete = $_POST['hiddenDelete'];		
					
		$statement = $conn->prepare("DELETE FROM users WHERE id = :clientToDelete");
		$statement->bindParam(":clientToDelete", $clientToDelete);
		try
		{
			$statement->execute();
...
<html>
	...
	<body>
		...
		<div id="content1" style="text-align: right;">
			<a href="addClient/">Add a new client account...</a></br></br>
				...
				<table style="width: 100%; text-align: center; border: solid 1px #dadada;">
					...
					<tr>
						...
						<td style="width: 20%; height: 40px;  border-bottom: solid 1px #dadada;">
							<h4><u>Delete Client:</u></h4>
						</td>
					</tr>
				
				<?php
					$statement = $conn->prepare("SELECT id, firstname, surname, username FROM users WHERE type = 1");
					$statement->execute();
							...
							<td style="width: 20%; height: 40px; border-right: solid 1px #dadada;">
								<p>
									<form action="editClient/" method="get">
										<input type="hidden" value="<?php echo $blocks['id']; ?>" name="clientToEdit" />
										<input type="submit" title="Click to edit client..." alt="Click to edit client..." value="<?php echo $blocks['username']; ?>" name="editClientSubmit" />
									</form>
								</p>
							</td>

<?php include('../../../../dbaccess/index.php');  include('../../../../dbaccess/modules.php');
	...
	{		
		try
		{
			...
			$statement = $conn->prepare("INSERT INTO blog (posted_by, title, content, tags) VALUES (:user, :postTitle, :postContent, :postTags)");
						...
						<tr>
							<td style="width: 70%; text-align: left; height: 40px; padding: 10px; border-right: solid 1px #dadada;">
								<form action="" method="post">
									...
									<input type="text" tag="Please make sure that you do not use a name already assigned to another post." alt="Please make sure that you do not use a name already assigned to another post." name="postTitle" value="<?php if(isset($_POST['postTitle'])){ echo $_POST['postTitle']; } ?>" /></br></br>
									<label>Content:</label>
									<textarea name="postContent" id="fblValue" class="wallPost" onkeydown="insertTab(this, event);" ><?php if (isset($_POST['postContent'])) { echo $_POST['postContent']; } ?></textarea>
									<label>Keywords/Tags:</label>
									<input type="text" tag="Seperate keywords with a comma(,)" alt="Seperate keywords with a comma(,)" name="tags" value="<?php if(isset($_POST['tags'])){ echo $_POST['tags']; } ?>" />
									<input type="submit" name="newpostSubmit" value="Save post"/>
								</form>
							</td>

							<td style="padding: 10px; width: 30%; height: 40px; text-align: left;">

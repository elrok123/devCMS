<?php include('../../../../dbaccess/index.php');  include('../../../../dbaccess/modules.php');
	...
	{		
		...
		try
		{
			$statement = $conn->prepare("INSERT INTO pages (Visible, pageName, type, link, linkLocation, topMenuVisible) VALUES (:pageVisible, :pageToSave, :type, :linkValue, :linkLocation, :topMenuVisible)");
			$statement->bindParam(":pageVisible", $pageVisible);
			$statement->bindParam(":pageToSave", $pageToSave);
			$statement->bindParam(":type", $type);
			$statement->bindParam(":linkValue", $linkValue);
			$statement->bindParam(":linkLocation", $linkLocation);
			$statement->bindParam(":topMenuVisible", $topMenuVisible);
						...
						<tr>
							<td style="width: 70%; text-align: left; height: 40px; padding: 10px; border-right: solid 1px #dadada;">
								<form action="" method="post">
									...
									<input type="text" tag="Please make sure that you do not use a name already assigned to another page/link." alt="Please make sure that you do not use a name already assigned to another page/link." name="pageName" value="" /></br>
									<label>Link Value: </label></br>
									<input type="text" value="" name="linkValue" /></br>
									<label>Select Link Location*: </label>
									<select name="linkLocation">
										<option value="1" selected> External</option>
										<option value="0">Internal</option>
										
									</select></br>
									</br>*This defines whether the link location content is actually stored on the server or whether it's external and stored on a seperate server</br></br>
									...
									<input type="submit" name="newPageSubmit" value="Save Link"/>
								</form>
							</td>

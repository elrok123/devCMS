...
for (var r = 0; r < $directories.length; r++) {
    e = e + "<span class='gallery-dir-click' id=\"" + $directories[r] + '"><img id="' + r + '" class="gallery-dir" src="' + $directories[r].replace(" ", "-") + ".jpg\" /><p class='gallery-dir-title'>" + $directories[r].substring(36, $directories[r].length) + "</p></span>";
    t = "";
...
$(".gallery-dir-click").click(function () {
    $("#portfolio-content-images").html($dirsJSON[$(this).attr("id")]);
    $(".gallery-image").click(function () {
        ...
        $(".galleria-next").click(function (e) {
            e.stopPropagation();
            if (parseInt($(this).attr("data-id")) >= $images.length) return false;
            var t = $("#" + $(this).attr("data-id") + ".gallery-image").attr("src");
            $("#lightbox-image").attr("src", t);
            if ($("#lightbox-image").width() > $("#lightbox-image").height()) $("#lightbox-image").css({
                width: window.innerWidth / 100 * 80 + "px",
